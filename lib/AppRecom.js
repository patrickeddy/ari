...
var AppRecom = function () {
  ...
  /**
   * <p>Train the recommender against a data set.</p>
   * <p>This method returns a JavaScript Promise.</p>
   *
   * <p>Entries in the data array look like:</p>
   * <p>
   ...
   * @param {Decimal} min_support - the minimum support percentage for an itemset
   * @param {Decimal} min_conf - the minimum confidence percentage for a rule
   * @return {Promise}
   */
  ...
  _createClass(AppRecom, [{
     ...
     * <p>[train()]{@link AppRecom#train} must be called before this function.</p>
    ...
    value: function getApps(location) {
      ...
      return new Promise(function (res, rej) {
        ...
        fs.readFile(_this2.rulesDirectory + RULES_FILENAME, RULES_ENCODING, function (err, data) {
          if (err) {
            print("Error finding " + RULES_FILENAME + ". Did you first call the train method?\n\n");rej(err); // error if couldn't read
          } else {
