...
var MIN_SUPPORT = 0.03;
var MIN_CONFIDENCE = 0.8;
var TEST_RATIO = 0.9;
var DEBUG = true; // Toggle this true or false for debug logs

// FILE PATHS
var DATA_FILE = "data.csv";
var RULES_FILE = "./node_modules/apprecom/apprecom_rules.txt";
...
function prompt() {
  ...
  rl.question(": ", function (answer) {
    if (hasSubStr(answer, "1")) {
      ...
      fs.readFile(RULES_FILE, "utf-8", function (err, data) {
        if (err) {
          print("\nError finding rules. Did you first call the train method?\n\n");
          prompt();
        } else {
          var locations = Object.keys(JSON.parse(data));
...
function readDataAndMine() {
  ...
  fs.readFile(DATA_FILE, CSV_ENCODING, function (err, data) {
    ...
    csv.parse(data.replace(/,\s/g, ","), CSV_OPTIONS, function (err, data) {
      ...
      // TRAIN RECOMMENDER
      AR.train(data, MIN_SUPPORT, MIN_CONFIDENCE, TEST_RATIO).then(function () {
        print("Trained!");
        prompt();
      });
    });
