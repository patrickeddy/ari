...
var AppRecom = function () {
  ...
  _createClass(AppRecom, [{
    ...
    value: function train(data) {
      ...
      var testRatio = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0.6;
      ...
      return new Promise(function (res, rej) {
        ...
        var firstStart = 0;
        var firstEnd = Math.round(numTraining / 2);
        var secondStart = data.length - Math.round(numTraining / 2) + 1;
        var secondEnd = data.length;
        if (_this.DEBUG) print("Training dimens: [[" + firstStart + ", " + firstEnd + "], [" + secondStart + ", " + secondEnd + "]]");
        var trainingSet = data.slice(firstStart, firstEnd); // add a bit of the first bit
        trainingSet = trainingSet.concat(data.slice(secondStart, secondEnd)); // add the last bit
        var trainingItemset = _this._getOptimalItemset(trainingSet, min_support);
        var trainingRules = _this._getRules(trainingSet, trainingItemset, min_conf);

        // Testing data
        var testStart = Math.round(numTraining / 2); // start somewhere in the beginning
        var testEnd = Math.round(data.length - numTraining / 2); // end somewhere near the end
        var testingItemset = data.slice(testStart, testEnd);

        // Logs
        if (_this.DEBUG) print("==============\nERROR TEST\ndata length: " + data.length + " - training length: " + trainingSet.length + " - testing length: " + testingItemset.length + "\n");

        // Get the error rate for this data.
        var errorrate = _this._testTrainingSet(trainingRules, testingItemset);
        if (_this.DEBUG) print("Error rate: " + errorrate);
